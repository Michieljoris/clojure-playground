...
(defn test "my doc for test" [] "test")
...
(loop [i 0 ret ["bla"] bla 2 foo 4]
  (if (< i 10)
    (recur (inc i) (conj ret i) 4 5)
    [ret bla foo i]))
...
(defmulti parse (fn [[f & r ]] f))

(defmethod parse 'if
  [form] {:op :if})

(defmethod parse 'let
  [form] {:op :let :form form})
